

<div align="center">

```
✦ MISSION FILE · PURVI-1508 · CLEARANCE: UNLIMITED ✦
```

<h1>
  <img src="https://readme-typing-svg.demolab.com?font=Orbitron&weight=900&size=32&pause=1000&color=4A9EFF&center=true&vCenter=true&width=600&lines=PURVI+VERMA;AI+ENGINEER+%7C+LLM+ARCHITECT;NEURAL+EXPLORER+%F0%9F%9A%80" alt="Purvi Verma" />
</h1>

<p><em>Because great AI isn't just powerful — it's safe.</em></p>

![Bangalore](https://img.shields.io/badge/📍-Bangalore,_India-020b18?style=for-the-badge&labelColor=0a2540&color=4a9eff)
![Company](https://img.shields.io/badge/🏢-Publicis_Sapient-020b18?style=for-the-badge&labelColor=0a2540&color=a78bfa)
![College](https://img.shields.io/badge/🎓-IIT_Bhubaneswar-020b18?style=for-the-badge&labelColor=0a2540&color=2dd4bf)
![Status](https://img.shields.io/badge/🔥-Open_to_Collabs-020b18?style=for-the-badge&labelColor=0a2540&color=f472b6)

</div>

---

## Mission Log

```yaml
CALLSIGN  : Purvi Verma
ROLE      : Senior Associate Data Scientist @ Publicis Sapient
ORBIT     : Bangalore, India 
EXPERTISE : LLM Safety · Multi-agent Orchestration · CV · NLP
SIDE_QUEST: Open Source · PyPI Publisher · Hackathon Chaser
```

---

## Active Missions

<table>
<tr>
<td width="50%">

### AI GUARDRAILS
**Safety system for Mondelez**
CV + LLMs + bring-your-own-guardrail architecture.
MCP tools for fine-grained policy enforcement.

![STATUS](https://img.shields.io/badge/●-LIVE-065f46?style=flat-square&labelColor=052e16&color=2dd4bf)

</td>
<td width="50%">

### AGRIBOT
**Multimodal AI for Indian farmers**
Voice · image · text · LangGraph + Gemini.
Real-time scheme matching + plant disease detection.

![STATUS](https://img.shields.io/badge/◈-GCP_FINALIST-3730a3?style=flat-square&labelColor=1e1b4b&color=a78bfa)

</td>
</tr>
<tr>
<td width="50%">

### [CODEMAP](https://github.com/purvi1508/CodeMap)
**Reverse-engineer any codebase**
Drop a repo → get architecture diagrams + AI docs.
Perfect for legacy code and developer onboarding.

![STATUS](https://img.shields.io/badge/◎-OPEN_SOURCE-1e3a8a?style=flat-square&labelColor=0c1a3a&color=4a9eff)

</td>
<td width="50%">

### [SAFEGUARD TOOLKIT](https://github.com/purvi1508/Safeguard)
**Python security scanner on PyPI**
Static analysis, permission audits, eval/exec detection.
CI/CD ready. `pip install safeguard_toolkit`

![STATUS](https://img.shields.io/badge/◎-ON_PYPI-1e3a8a?style=flat-square&labelColor=0c1a3a&color=4a9eff)

</td>
</tr>
<tr>
<td width="50%">

### ATOMS
**Scientific poster extraction**
Structured data from posters (titles, authors, figures).
Built with Pfizer & Eli Lilly using vision + NLP.

![STATUS](https://img.shields.io/badge/●-DEPLOYED-065f46?style=flat-square&labelColor=052e16&color=2dd4bf)

</td>
<td width="50%">

### NEURAL PLASTICITY
**Thermodynamic NN research**
Chaboche-type hardening in Julia (Flux.jl).
Supervised by IIT Bhubaneswar + IIT Bombay.

![STATUS](https://img.shields.io/badge/◈-IN_PROGRESS-3730a3?style=flat-square&labelColor=1e1b4b&color=a78bfa)

</td>
</tr>
</table>

---

## Onboard Systems

```
COGNITION   →  Claude · GPT-4 · Gemini · Llama · HuggingFace OSS
NAVIGATION  →  LangGraph · LangChain · LangFlow · MLFlow
SENSORS     →  PyTorch · TensorFlow · Keras · Scikit-learn · OpenCV
PROPULSION  →  Docker · Kubernetes · GCP · Firebase · Vertex AI
LANGUAGES   →  Python · C++ · Julia (Flux.jl)
STORAGE     →  MySQL · MongoDB · Firestore
```

---

## Medals of Honor

| Achievement |
|---|
| **Finalist** — Google Cloud Agentic AI Day Hackathon · AgriBot |
| Published [`safeguard_toolkit`](https://pypi.org/project/safeguard_toolkit/) on PyPI |
| Contributor · `video-keyframe-detector` · `CRAFT-pytorch` (PyTorch OCR inference stability) |

---

## GitHub Stats

<div align="center">

![Purvi's GitHub Stats](https://github-readme-stats.vercel.app/api?username=purvi1508&show_icons=true&theme=tokyonight&hide_border=true&bg_color=020b18&title_color=4a9eff&icon_color=a78bfa&text_color=94a3b8)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=purvi1508&layout=compact&theme=tokyonight&hide_border=true&bg_color=020b18&title_color=4a9eff&text_color=94a3b8)

![GitHub Streak](https://streak-stats.demolab.com?user=purvi1508&theme=tokyonight&hide_border=true&background=020b18&ring=4a9eff&fire=f472b6&currStreakLabel=a78bfa)


**◈ OPEN CHANNEL ◈**

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-4a9eff?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/purvi-verma-3a553a23b/))
[![Portfolio](https://img.shields.io/badge/Portfolio-Visit-a78bfa?style=for-the-badge)](https://purvi1508.github.io/PurviVerma/)
[![LeetCode](https://img.shields.io/badge/LeetCode-Solve-2dd4bf?style=for-the-badge&logo=leetcode&logoColor=white)](https://leetcode.com/u/20ce01050/)

![Visitor Count](https://komarev.com/ghpvc/?username=purvi1508&style=for-the-badge&color=4a9eff&label=MISSION+VISITORS)

</div>
